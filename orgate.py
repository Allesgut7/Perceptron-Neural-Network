...
def update_weights(x, y, W, learning_rate): #mendefinisikan fungsi update bobot dengan menggunakan turunan sigmoid
    Z = np.dot(x, W)
    Y_hat = sigmoid(Z) #output aktual
    error = y - Y_hat  #selisih antara output yang diinginkan (y) dan output aktual (Y_hat).
    dEdW = error * derivative_sigmoid(Z) * x #gradient error
    W += learning_rate * dEdW #pembaruan bobot
    return W, error 
...
label_iterations_before_update = tk.Label(frame_inputs, text="Iterasi :")
